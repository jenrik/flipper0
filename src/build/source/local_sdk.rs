...
pub fn try_build() -> Result {
	...
	let (sdk_tags, sdk_rev) = match validate_sdk(&root) {
		...
		Err(err) => {
			println!("cargo:warning=Unknown SDK, cannot validate: {err}");
			(vec![], None)
		},
	};
	...
	let exclusions = exclusions([&header, &extra])?;
...
impl bindgen::callbacks::ParseCallbacks for DeriveCallbacks<PathBuf> {
	...
	fn add_derives(&self, name: &str) -> Vec<String> {
		let debug = crate::is_debug() || crate::feature("derive-debug");
		if ["FS_Error"].contains(&name) && !debug {
			vec!["Debug".to_owned()]
		} else if ["FileInfo"].contains(&name) && !debug {
			vec!["Clone".to_owned()]
		} else {
			vec![]
		}
...
fn from_source<P: AsRef<Path>, Builder: FnOnce() -> bindgen::Builder>(builder: Builder,
	...
	if try_exists(symbols_header)? {
		builder = builder.header(format!("{}", symbols_header.display()))
		                 ...
		                 .clang_arg(format!("-I{}", &PathBuf::from("lib/stm32wb_hal/Inc").display()))
		                 .clang_arg(format!("-I{}", &PathBuf::from("lib/stm32wb_cmsis/Include").display()))
		                 .clang_arg("-DSTM32WB")
